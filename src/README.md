...
Log files are stored in a specified directory in files with a well defined filename. The current journal is called `current`. When full, it is renamed in the format `{prefix}.{n}.log` where `{prefix}` is specified on the command line and `{n}` is a zero padded 3 digit number, starting at 1.
...
```
Creates log files foo.001.log, foo.002.log etc. in the `logfiles` folder.

```sh
cat somebiglog | rollout -p bar -k 2 -s 1024 logs
```
Creates log files bar.001.log, bar.002.log etc. in the `logfiles` folder. Only keep the last 2 complete logs, and each log file should be around 1024KB in size.